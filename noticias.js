...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const NEWS_API_KEY = 'b9b6a0ebfd5c4a0cb288748ea1d9df7e'; // Substitua pela sua chave

    const NEWS_SOURCES_CONFIG = [
        { url: `https://newsapi.org/v2/everything?q=world&language=en&sortBy=publishedAt&pageSize=10&apiKey=${NEWS_API_KEY}` }, // Aumentei o pageSize para ter mais opções
    ...
    function renderNews(newsItems) {
        ...
        newsItems.forEach(news => {
            // Sua lógica para renderizar as notícias na grade continua aqui
            const article = document.createElement('article');
            article.classList.add('news-card');
    ...
    async function fetchAndDisplayLatestNews() {
        ...
        renderNews(allNews); // A função renderNews agora cuida da notícia em destaque
    }

    fetchAndDisplayLatestNews();

    // ... (seu código para o botão "Voltar ao Topo" continua aqui)
});
